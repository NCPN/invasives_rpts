dbMemo "SQL" ="SELECT DISTINCT Master_Plant_Code_FK, LU_Code, Family, Species_Name, utah_specie"
    ...
    ", \"qry_Annual_Complete_Tgt_Species_Lists\",\"Park= 'GOSP' AND Species_Name='\"+"
    "Species_Name+\"'\",'',\"|\") AS ParkYearPriorities, (SELECT Min(TgtYear) FROM qr"
    "y_Annual_Complete_Tgt_Species_Lists WHERE Park = 'GOSP') AS MinYear, (SELECT Max"
    "(TgtYear) FROM qry_Annual_Complete_Tgt_Species_Lists WHERE Park = 'GOSP') AS Max"
    "Year\015\012FROM (SELECT * FROM qry_Annual_Complete_Tgt_Species_Lists WHERE Park"
    " = 'GOSP')  AS [%$##@_Alias]\015\012GROUP BY Park, Master_Plant_Code_FK, LU_Code"
